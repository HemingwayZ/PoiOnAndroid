...
public final class PackagePartName implements Comparable<PackagePartName> {
	...
	PackagePartName(URI uri, boolean checkConformance)
		...
		if (checkConformance) {
			...
			if (!org.apache.poi2.openxml4j.opc.PackagingURIHelper.PACKAGE_ROOT_URI.equals(uri)) {
				throw new org.apache.poi2.openxml4j.exceptions.OpenXML4JRuntimeException(
	...
	PackagePartName(String partName, boolean checkConformance)
		...
		if (checkConformance) {
			...
			if (!org.apache.poi2.openxml4j.opc.PackagingURIHelper.PACKAGE_ROOT_URI.equals(partURI)) {
	...
	private boolean isRelationshipPartURI(URI partUri) {
		...
		return partUri.getPath().matches(
				"^.*/" + org.apache.poi2.openxml4j.opc.PackagingURIHelper.RELATIONSHIP_PART_SEGMENT_NAME + "/.*\\"
						+ org.apache.poi2.openxml4j.opc.PackagingURIHelper.RELATIONSHIP_PART_EXTENSION_NAME
						+ "$");
	}
	...
	private static void throwExceptionIfEmptyURI(URI partURI)
		...
		if (uriPath.length() == 0
				|| ((uriPath.length() == 1) && (uriPath.charAt(0) == org.apache.poi2.openxml4j.opc.PackagingURIHelper.FORWARD_SLASH_CHAR)))
		...
		for (int i = 0; i < segment.length(); ++i) {
			...
			if (errorFlag && c == '%') {
				...
				if (((segment.length() - i) < 2)) {
					throw new org.apache.poi2.openxml4j.exceptions.InvalidFormatException("The segment " + segment
	...
	private static void throwExceptionIfPartNameNotStartsWithForwardSlashChar(
			URI partUri) throws org.apache.poi2.openxml4j.exceptions.InvalidFormatException {
		String uriPath = partUri.getPath();
		if (uriPath.length() > 0
				&& uriPath.charAt(0) != org.apache.poi2.openxml4j.opc.PackagingURIHelper.FORWARD_SLASH_CHAR)
			throw new org.apache.poi2.openxml4j.exceptions.InvalidFormatException(
